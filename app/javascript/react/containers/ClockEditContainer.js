...
class ClockEditContainer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name: '',
  ...
  handleFormSubmit(event) {
    ...
    if (errors.length ===0) {
      fetch(`/api/v1/games/${this.state.gameId}/clocks/${this.props.params.id}`, {
      ...
      .then(response => {
        ...
        browserHistory.push(`/clocks/${this.props.params.id}`)
      })
        ...
        <div className= "panel small-12 large-8 small-centered columns" >
          ...
          <form onSubmit = {this.handleFormSubmit}>
            ...
            <label>Starting ticks
              <input type="number" value={this.state.ticks} onChange = {this.handleTicksChange}/>
            </label>
            ...
            <div className="small-8 columns small-centered small-block-grid-2">
              ...
              <Link to={"/games/"+this.state.gameId+"/clocks"} className = "button">Cancel</Link>
            </div>
          </form>
        </div>
      </div>
...
export default ClockEditContainer
