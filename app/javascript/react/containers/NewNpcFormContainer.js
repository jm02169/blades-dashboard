...
class NewNpcFormContainer extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name: '',
  ...
  handleFormSubmit(event) {
    ...
    if (errors.length ===0) {
      fetch(`/api/v1/games/${this.props.params.id}/npcs`, {
      ...
      .then(response => {
        ...
        browserHistory.push(`/games/${this.props.params.id}/npcs`)
      })
        ...
        <div className= "panel small-12 large-8 small-centered columns" >
          ...
          <form onSubmit = {this.handleFormSubmit}>
            ...
            <div className="small-8 columns small-centered small-block-grid-2">
              ...
              <Link to={"/games/"+this.props.params.id+"/npcs"} className = "button">Cancel</Link>
            </div>
          </form>
        </div>
      </div>
...
export default NewNpcFormContainer
